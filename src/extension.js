...
const checkExtensionVersion = function () {
  try {
    const installedExtensions = fs.readdirSync(targetDir, { withFileTypes: true })
      .filter(dirent => dirent.isFile()).map(dirent => dirent.name)
    if (!installedExtensions.includes(scriptName)) { return false }

    const latestVersion = getVersion(sourceDir)
    const installedVersion = getVersion(targetDir)
...
const copyScripts = async function () {
  const scripts = (await readdir(sourceDir, { withFileTypes: true }))
    .filter(dirent => dirent.isFile()).map(dirent => dirent.name)
